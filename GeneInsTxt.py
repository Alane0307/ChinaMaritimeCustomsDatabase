...
with open('InseTxt.txt', 'w') as f1:
    ...
    for row in rows:
        ...
        if not row[7].value:
            ...
            f1.write('\''+row[7].value.encode('utf8')+'\', ')
            
        if not row[8].value:
            f1.write('NULL, ')
        else:
            f1.write('\''+str(row[8].value)+'\', ')
            
        if not row[9].value:
            f1.write('NULL, ')
        else:
            f1.write('\''+str(row[9].value)+'\', ')
        ...
        if not row[11].value:
            ...
            f1.write('\''+row[11].value.encode('utf8')+'\', ')
            
        if not row[12].value:
            f1.write('NULL, ')
        else:
            f1.write('\''+str(row[12].value)+'\', ')
            
        if not row[13].value:
            f1.write('NULL, ')
        else:
            f1.write('\''+row[13].value.encode('utf8')+'\', ')
            
        if not row[14].value:
            f1.write('NULL),\n ')
        else:
            f1.write('\''+row[14].value.encode('utf8')+'\'),\n')
